...
var <?=JVAR?> = {
    ...
    edit:function(id){
        var that = this, href = '<?=url('schedulers/edit')?>?id='+(id?id:'');
        $(that.dialog).dialog({
            title: '计划任务',
    ...
    remove:function(id){
        ...
        $.messager.confirm('提示', '确认删除吗?', function(result){
            ...
            $.messager.progress({text:'处理中，请稍候...'});
            $.post('<?=url('schedulers/remove')?>', {id:id}, function(res){
                $.messager.progress('close');
                if(!res.code){
                    $.app.method.alertError(null, res.msg);
                }else{
    ...
    detail:function(idx, row){
        ...
        $(that.dialog).dialog({
            title: '计划任务 - ' + row.name,
            iconCls: 'fa fa-eye',
            width: <?=$loginMobile?"'90%'":800?>,
            height: '95%',
            cache: false,
            href: '<?=url('schedulers/view')?>?id='+row.id,
            modal: true,
            collapsible: false,
            minimizable: false,
            resizable: false,
    ...
    logs:function(id, title){
        ...
        $(that.dialog).dialog({
            ...
            height: '95%',
            cache: false,
            href: '<?=url('schedulers/view')?>?id='+id,
            modal: true,
            collapsible: false,
            minimizable: false,
            resizable: false,
