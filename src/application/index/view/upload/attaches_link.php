<span id="attaches_{$uniqid}">
    <?php foreach($bindValues['attaches'] as $attach){ ?>
        <div id="attach_file_{$uniqid}_<?=$attach['attachment_id']?>" class="attach-box-light">
            <div class="text-center mr-5 pull-left">
        ...
        uploadAttach:function(){
            ...
            $.app.method.upload(url, function (obj) {
                ...
                $.each(obj.data,function(i,v){
                    html.push(
                        '<div id="attach_file_{$uniqid}_' + v.attachment_id + '" class="attach-box-light">' +
                        '<div class="text-center mr-5 pull-left"><a title="' + v.name + '" href="javascript:void(0)" onclick="QT.filePreview('+v.attachment_id+')">'+
                        v.name + '</a></div>' +
                        '<div class="text-center pull-left">' +
